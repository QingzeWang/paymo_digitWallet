...
class paymo(object):
	...
	'''
	This class: paymo, is used to build up and analyze a graph to represent users' relationship by using the DICTIONARY data structure. The following four methods are included for an paymo_conn object: 1, paymoGraph　==> Build users' graph based on current transaction records. 2, paymoDegree  ==> Calculate the degree between any two users up to 4. 3, paymoOutput ==> Calculate the number of degree for each record in stream_payment.txt and generate 3 different output files. 4, flatten_lists ==> flatten a nested list_of_list to a list
	'''
	...
	# Flatten a nested list: a short function to change a list_of_list to a list 
	...
	def paymoDegree(self, graph, usr1, usr2):
		...
		dist3 = self.flatten_lists([self.graph[key3] for key3 in dist2])# find a 3rd degree friend
 		if (usr1 not in dist2) and (usr1 in dist3):
			degree = 3
			return degree #degree is 3     

		dist4 = self.flatten_lists([self.graph[key4] for key4 in dist3])# find a a 4th degree friend 
	...
	def paymoOutput(self,graph,streamFile,outFile1,outFile2,outFile3):
		...
		print("Average elapsed time of one transaction for all 3 cases together is {0:.2e} s".format(time_used))
